...
const AdminMerch: React.FC = () => {
  const [isCreating, setIsCreating] = useState(false);
  const merch = [
    { id: 1, name: '"Unlocking Truth" T-Shirt', link: 'https://teepublic.com/...' },
    { id: 2, name: '"Wisdom Seeker" Hoodie', link: 'https://teepublic.com/...' },
  ];

  const FormView = () => (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Add New Merch</h1>
      <form className="space-y-6 bg-white dark:bg-gray-800 p-8 rounded-xl shadow-lg">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Product Name</label>
          <input type="text" className="w-full input-class" placeholder="Enter product name" />
        </div>
        <div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">TeePublic Link</label>
          <input type="url" className="w-full input-class" placeholder="https://teepublic.com/..." />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Description</label>
          <textarea rows={4} className="w-full input-class" placeholder="Short description of the product"></textarea>
        </div>
        <div className="flex space-x-4">
          <button type="submit" className="btn-primary">Add Merch</button>
  ...
  const ListView = () => (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Manage Merch</h1>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 dark:bg-gray-700">
              ...
              <th className="th-class">TeePublic Link</th>
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {merch.map(item => (
              <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                <td className="td-class font-medium">{item.name}</td>
                <td className="td-class truncate max-w-xs">{item.link}</td>
